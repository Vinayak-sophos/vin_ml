...
class model:
	...
	def find_information_gain_ratio(self, feature):
		return self.find_information_gain(feature, self.target) / self.find_entropy_whole_dataset(self.data, feature)
	...
	def build(self):
		...
		for feature in self.features:
			...
			print("IGR (", feature, ") =", information_gain_ratio[feature])
	...
	def plot(self, graph, parent=None, parent_label=None, parent_child_edge=None, query=None):
		if self.is_leaf:
			...
			index = self.feature_index[self.max_igr_feature]%len(NODE_FILL_COLOR)
			if parent_label:
				if query and query[parent] == parent_child_edge:
				...
				for level in self.levels[self.max_igr_feature]:
					if query and level == query[self.max_igr_feature]:
						self.children[level].plot(graph, parent=self.max_igr_feature, parent_label=parent_label + parent_child_edge + self.max_igr_feature, parent_child_edge=level, query=query)
					else:
						self.children[level].plot(graph, parent=self.max_igr_feature, parent_label=parent_label + parent_child_edge + self.max_igr_feature, parent_child_edge=level)
			else:
				if query:
				...
				for level in self.levels[self.max_igr_feature]:
					if query and level == query[self.max_igr_feature]:
						self.children[level].plot(graph, parent=self.max_igr_feature, parent_label=self.max_igr_feature, parent_child_edge=level, query=query)
					else:
						self.children[level].plot(graph, parent=self.max_igr_feature, parent_label=self.max_igr_feature, parent_child_edge=level)
